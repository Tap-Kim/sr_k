...
const EST_ENGINEER_2: PositionExpeirence = {
  ...
  contents: [
    ...
    [
      "기존 서비스의 다국어 적용 시 엑셀에 정의된 내용을 복사/붙여넣기시 발생하는 작업으로 인한, 번역 Key 누락 및 중복, 오타 문제 등 휴먼 에러가 반복되어 불필요한 소통 시간 문제 제기",
...
const EST_ENGINEER_3: PositionExpeirence = {
  ...
  contents: [
    ...
    [
      "자사 서비스들의 디자인 시스템 부재로 인한 UX와 DX 저하를 경험하여 스쿼드에 도입을 제안",
      "Perso 도메인 내 서비스 확장 여부와 브랜드 이미지를 확립하기 위해 디자인 팀과 핸드오프를 적극 주도",
...
const EST_ENGINEER_4: PositionExpeirence = {
  ...
  contents: [
    ...
    [
      ...
      "Atomic/VAC 디자인 패턴 도입 후 개발/디자인 작업 병렬 프로세스 구축하여 작업 생산성 확보",
    ],
    {
      bold: true,
    ...
    [
      "패키지 종속성 관리 이슈 해결을 위해 Yarn-Berry 기반 pnp 방식 적용으로 의존성 충돌 제거",
      "디자인 접근성, 확장성을 위해 Bootstrap > MUI 전환 및 인터페이스 재정의로 UI 일관성 개선",
      "테이블, 검색, 모달 등 반복되는 컴포넌트를 Formik 조합 컴포넌트 템플릿화로 중복 코드 제거 및 유지보수성 개선",
      // "",
      // "",
      // "",
      // "",
...
const DOUZNE_DEVELOPER_1: PositionExpeirence = {
  ...
  contents: [
    ...
    [
      ...
      "웹 서비스 대비 전체적으로 모바일 앱 성능 60% 이상 개선 및 Indicator 처리를 통한 UX 개선",
    ],
    ...
    [
      "Expo 프레임워크의 파일 업/다운로드 기능 부재로 인해 타 모바일 서비스 개발 지연 문제 확보",
